    ...
    <footer className="bg-gray-900 text-gray-400">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
          ...
          <div>
            ...
            <div className="flex gap-3">
              {[FiFacebook, FiTwitter, FiInstagram].map((Icon, i) => (
                <a key={i} href="#"
                  className="w-8 h-8 rounded-full bg-gray-800 flex items-center justify-center hover:bg-green-400 hover:text-gray-900 transition-colors">
                  <Icon size={14} />
                </a>
              ))}
            </div>
              ...
              {['Cement', 'Concrete', 'Aggregates', 'Dry Mix', 'White Cement', 'Ready Mix'].map((cat) => (
                <li key={cat}>
                  <Link to={`/products?category=${cat}`}
                    className="hover:text-green-400 transition-colors">{cat}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm">
              <li className="flex items-start gap-2">
                <FiMapPin className="text-green-400 mt-0.5 flex-shrink-0" size={14} />
              ...
              <li className="flex items-center gap-2">
                ...
                <span>support@cement2steel.com</span>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="border-t border-gray-800 mt-10 pt-6 flex flex-col sm:flex-row justify-between items-center gap-3 text-xs">
          <p>© {new Date().getFullYear()} Cement2Steel Ltd. All rights reserved.</p>
